	 ...
	 * @param groupid
	 */
	function addAStudentToGroup(data,leaderid,groupid,courseid){
		var credit;
		if(data.credit==null)
			credit="未评分";
		...
		if(data.id==leaderid){
		 inputdata = '\
			<tr id="tr-'+leaderid+'">\
				...
				<td>组长</td>\
				<td>'+credit+'</td>\
				<td>'+data.isSubmitChinese+'</td>\
				<td><a href="/classInfo/tch/evaluateGroupMission4personal.action?courseid='+courseid+'&cmissionid='+data.cmissionid+'&cmstudentid='+data.id+'&cstudentid='+data.cstudentid+'">评分</a></td>\
				<td>\
				</td>\
			</tr>\
		';
...
//						resetForm();
						...
						for(var i = data.cmstudentDetailViews.length-1;i>=0;i--){
							addAStudentToGroup(data.cmstudentDetailViews[i],data.leaderid,data.cmgroupid,data.courseid);
						}
//						autoIndex();
//					   	tableGap();
					}
